  ...
  [filename]:
    ...
    .addRow(
      ...
      .addPanel(
        ...
        $.statPanel('sum(cortex_ruler_managers_total{%s})' % $.appMatcher($._config.app_names.ruler), format='short')
      )
      .addPanel(
        $.panel('Total rules') +
        $.statPanel('sum(cortex_prometheus_rule_group_rules{%s})' % $.appMatcher($._config.app_names.ruler), format='short')
      )
      .addPanel(
        $.panel('Read from ingesters - QPS') +
        $.statPanel('sum(rate(cortex_ingester_client_request_duration_seconds_count{%s, operation="/cortex.Ingester/QueryStream"}[$__rate_interval]))' % $.appMatcher($._config.app_names.ruler + $._config.app_names.ruler_querier), format='reqps') +
        $.panelDescription(
      ...
      .addPanel(
        ...
        $.statPanel('sum(rate(cortex_ingester_client_request_duration_seconds_count{%s, operation="/cortex.Ingester/Push"}[$__rate_interval]))' % $.appMatcher($._config.app_names.ruler), format='reqps')
      )
    )
    .addRow(
    ...
    .addRowIf(
      ...
      .addPanel(
        $.panel('QPS') +
        $.qpsPanel('cortex_request_duration_seconds_count{%s, route=~"%s"}' % [$.appMatcher($._config.app_names.gateway), ruler_config_api_routes_re])
      )
      .addPanel(
        $.panel('Latency') +
        utils.latencyRecordingRulePanel('cortex_request_duration_seconds', $.appSelector($._config.app_names.gateway) + [utils.selector.re('route', ruler_config_api_routes_re)])
      )
      .addPanel(
        local selectors = $.appSelector($._config.app_names.gateway) + [utils.selector.re('route', ruler_config_api_routes_re)];
    ...
    .addRow(
      ...
      .addPanel(
        $.panel('QPS') +
        $.qpsPanel('cortex_ingester_client_request_duration_seconds_count{%s, operation="/cortex.Ingester/Push"}' % $.appMatcher($._config.app_names.ruler))
      )
      .addPanel(
        $.panel('Latency') +
        $.latencyPanel('cortex_ingester_client_request_duration_seconds', '{%s, operation="/cortex.Ingester/Push"}' % $.appMatcher($._config.app_names.ruler))
      )
    )
    .addRow(
      $.row('Reads (ingesters)')
      .addPanel(
        $.panel('QPS') +
        $.qpsPanel('cortex_ingester_client_request_duration_seconds_count{%s, operation="/cortex.Ingester/QueryStream"}' % $.appMatcher($._config.app_names.ruler + $._config.app_names.ruler_querier))
      )
      .addPanel(
        $.panel('Latency') +
        $.latencyPanel('cortex_ingester_client_request_duration_seconds', '{%s, operation="/cortex.Ingester/QueryStream"}' % $.appMatcher($._config.app_names.ruler + $._config.app_names.ruler_querier))
      )
    )
    .addRowIf(
    ...
    .addRow(
      ...
      .addPanel(
        ...
        $.latencyPanel('cortex_querier_storegateway_instances_hit_per_query', '{%s}' % $.appMatcher($._config.app_names.ruler), multiplier=1) +
        { yaxes: $.yaxes('short') },
      )
      .addPanel(
        $.panel('Refetches of missing blocks per query') +
        $.latencyPanel('cortex_querier_storegateway_refetches_per_query', '{%s}' % $.appMatcher($._config.app_names.ruler), multiplier=1) +
        { yaxes: $.yaxes('short') },
      )
      .addPanel(
        $.panel('Consistency checks failed') +
        $.failurePanel('sum(rate(cortex_querier_blocks_consistency_checks_failed_total{%s}[$__rate_interval])) / sum(rate(cortex_querier_blocks_consistency_checks_total{%s}[$__rate_interval]))' % [$.appMatcher($._config.app_names.ruler), $.appMatcher($._config.app_names.ruler)], 'Failures / sec') +
    ...
    .addRow(
      ...
      .addPanel(
        ...
        $.queryPanel(|||
          ...
          > 0
        ||| % [$.appMatcher($._config.app_names.ruler), $.appMatcher($._config.app_names.ruler)], '{{ user }}')
      )
      .addPanel(
        $.panel('Queue length') +
        $.queryPanel(|||
      ...
      .addPanel(
        ...
        ||| % $.appMatcher($._config.app_names.ruler), '{{ user }}')
      )
    )
    .addRow(
      ...
      .addPanel(
        ...
        $.queryPanel('sum by(user) (rate(cortex_prometheus_rule_group_iterations_missed_total{%s}[$__rate_interval])) > 0' % $.appMatcher($._config.app_names.ruler), '{{ user }}'),
      )
      .addPanel(
        $.panel('Latency') +
        $.queryPanel(
          ...
          ||| % [$.appMatcher($._config.app_names.ruler), $.appMatcher($._config.app_names.ruler)],
          '{{ user }}'
        ) +
        { yaxes: $.yaxes('s') },
      )
      .addPanel(
        $.panel('Failures') +
        $.queryPanel(
          'sum by(rule_group) (rate(cortex_prometheus_rule_evaluation_failures_total{%s}[$__rate_interval])) > 0' % [$.appMatcher($._config.app_names.ruler)], '{{ rule_group }}'
        )
      )
    )
    .addRow(
